...
if os.path.dirname(__file__):           # load by import
    ...
    CONFIG_FILE = '/etc/masakari/masakari-instancemonitor.conf'
    # LOG_FILENAME=dir_path+'/eventCallback.log'
    # Log output destination
    LOG_FILENAME = ''
# If the log is not output
else:
    # load as main (debug)
    # Callback configuration file
    CONFIG_FILE = '/etc/masakari/masakari-instancemonitor.conf'
...
# Area that holds the set of callback
callback_config = {}
...
#    noticeID          : Unique ID for each notification contents notify
#                      : to the callback function
#    noticeType        : Notice type notify to the callback function
#    hostname          : Server host name of the source an event occur
#                      : notify to the callback function
#    currentTime       : Event occurred time notify to the callback function
...
def libvirtEventCallback(
        eventID, eventType, detail, uuID,
        noticeID, noticeType, hostname,
        currentTime, tz_name_str, time_daylight_str):
    ...
    evf.syslogout("libvirt Event: id=%s type=%s regionID=%s "
                  "hostname=%s uuid=%s time=%s eventID=%s eventType=%s"
                  "detail=%s tzname=%s daylight=%s cluster_port=null" % (
                      noticeID, noticeType, regionID,
                      hostname, uuID, currentTime, eventID,
                      eventType, detail, tz_name_str, time_daylight_str),
                  callback_config['logLevel'],
                  callback_config['logFacility']
                  )
    ...
    event = {'id': noticeID, 'type': noticeType, 'regionID': regionID,
             ...
             'tzname': tz_name_str, 'daylight': time_daylight_str,
             'cluster_port': ''}
    ...
    for i in range(int(callback_config['retry'])):
        ...
        # HTTP POST
        try:
            resp, content = post_event(
                url=http_url, event=event, retry_timeout=retry_timeout)
            # Response checking
            ...
            # Timeout
            logmsg("Error Response(" + str(i + 1) +
                   "): No response. Timeout.", True)
            pass
        except socket.error, e:
            # Connection failure
            logmsg("Error Response(" + str(i + 1) +
                   "): Socket error. (" + str(e) + ")", True)
            pass
        # "sleep" of the specified interval
        time.sleep(float(callback_config['interval']))
    # Retry over
    logmsg("HTTP Request Retry Over. url=%s id=%s type=%s regionID=%s"
           "hostname=%s uuid=%s time=%s eventID=%s eventType=%s"
           "detail=%s tzname=%s daylight=%s" % (
               http_url, noticeID, noticeType, regionID, hostname,
               uuID, currentTime, eventID, eventType, detail,
...
def callback_config_setup():
    ...
    callback_config = {'url': inifile.get('callback', 'url'),
                       ...
                       'retry_timeout': inifile.get('callback',
                                                    'retry_timeout')}
...
#
# Function overview:
#   The message is converted to JSON format.
#   And it is sent by the HTTP POST method.
...
#   retry_timeout : Timeout value of the processing
#                 : which depends of the notification once
#
# Return value:
#   Http().request result
#
...
if __name__ == "__main__":
    libvirtEventCallback(5, 10, 1, 'a-b-c', 'd-e-f',
                         'type', 'host', 'YYYYmmddHHMMSS')
